...
// Hierarchy: Brand > Collection > Family > Watch
db.exec(`
...
if (!watchCols.includes('family_id')) {
  db.exec('ALTER TABLE watches ADD COLUMN family_id INTEGER')
}
const orphanCollections = db.prepare('SELECT DISTINCT collection_id FROM watches WHERE family_id IS NULL').all()
if (orphanCollections.length) {
  const findFam = db.prepare('SELECT id FROM families WHERE collection_id=? AND name=?')
  const insFam = db.prepare('INSERT INTO families (collection_id,name) VALUES (?,?)')
  const setFam = db.prepare('UPDATE watches SET family_id=? WHERE collection_id=? AND family_id IS NULL')
  db.transaction(() => {
    orphanCollections.forEach(({ collection_id }) => {
      if (collection_id == null) return
      const existing = findFam.get(collection_id, 'General')
      const famId = existing ? existing.id : insFam.run(collection_id, 'General').lastInsertRowid
      setFam.run(famId, collection_id)
    })
  })()
}
...
function seed() {
  ...
  const insFam = db.prepare('INSERT INTO families (collection_id,name,description,sort) VALUES (?,?,?,?)')
  const insW = db.prepare('INSERT INTO watches (collection_id,family_id,name,values_json,sort) VALUES (?,?,?,?,?)')

  const data = [
    {
      brand: ['Seiko', 'Japan', '1881', 'https://www.seikowatches.com', 'Founded by Kintarō Hattori in Tokyo.'],
      collections: [
        { name: '5 Sports', desc: 'Affordable automatic sports watches.', families: [
          { name: 'Sports Style', watches: [
          ...
          { name: 'GMT', watches: [
            { name: 'SRPK21 "GMT"', v: { reference: 'SRPK21', status: 'Wishlist', movement: 'Automatic', caliber: '4R34', water_resistance: 100, case_size: 42.5, case_material: 'Stainless Steel', crystal: 'Hardlex', dial_color: 'Blue/Grey', price: 400 } },
          ] },
        ] },
        { name: 'Prospex', desc: 'Professional dive & sports line.', families: [
          { name: 'Turtle', watches: [
            { name: 'SRPE05 "Black Turtle"', v: { reference: 'SRPE05K1', status: 'Wishlist', movement: 'Automatic', caliber: '4R35', water_resistance: 200, case_size: 45, case_material: 'Stainless Steel', crystal: 'Hardlex', dial_color: 'Black', price: 380, rating: '★★★★★' } },
          ] },
        ] },
      ],
    },
    {
      ...
      collections: [
        { name: 'Promaster', desc: 'Air, land and sea tool watches.', families: [
          { name: 'Diver', watches: [
            { name: 'BN0150 "Fugu"', v: { reference: 'BN0150-28E', status: 'Considering', movement: 'Eco-Drive', water_resistance: 200, case_size: 44, case_material: 'Stainless Steel', crystal: 'Mineral', dial_color: 'Black', price: 250 } },
          ] },
        ] },
        { name: 'Tsuyosa', desc: 'Integrated-bracelet automatics.', families: [
    ...
    },
    {
      ...
      collections: [
        { name: 'G-Shock', desc: 'Shock-resistant digital & analog.', families: [
          { name: '2100 Line', watches: [
        ...
        { name: 'Lineage', desc: 'Slim solar radio-controlled dress watches.', families: [
          { name: 'Solar', watches: [
    ...
    },
    {
      ...
      collections: [
        { name: 'Heritage', desc: 'Nature-of-time dials.', families: [
          { name: 'Snowflake', watches: [
    ...
    data.forEach((b, bi) => {
      ...
      b.collections.forEach((c, ci) => {
        ...
        c.families.forEach((fam, fi) => {
          const fId = insFam.run(cId, fam.name, fam.desc || null, fi).lastInsertRowid
          fam.watches.forEach((w, wi) => insW.run(cId, fId, w.name, JSON.stringify(w.v || {}), wi))
        })
      })
    })
  })()
